...
async def test_get_many_by_app_id_filters_by_app_id_and_type(
    ...
    results = await repo.get_many_by_app_id(
        db_conn, "app-1", EvaluationType.LLM_ONLY
    )
    ...
    assert all(
        r.type == EvaluationType.LLM_ONLY for r in results
    )


@pytest.mark.anyio
async def test_get_many_by_app_id_returns_empty_list_when_no_match(
    db_conn: AsyncConnection, repo: EvaluationsRepository
):
    # Act
    results = await repo.get_many_by_app_id(
        db_conn, "nonexistent", EvaluationType.LLM_ONLY
    )
