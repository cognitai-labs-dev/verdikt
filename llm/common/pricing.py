...
class PricingService:
    def get_input_token_cost(self, model: str, input_tokens: int) -> float:
        model_pricing = self._get_model_pricing(model)
        return input_tokens * model_pricing.input_cost_per_token

    def get_output_token_cost(self, model: str, output_tokens: int) -> float:
        model_pricing = self._get_model_pricing(model)
    ...
    def _get_model_pricing(model: str) -> TokensPricing:
        ...
        if model not in model_cost:
            raise ValueError(f"Model {model} not found in litellm pricing data")
    ...
    def get_response_stats(self, response: Response, model_name: str) -> ResponseStats:
        ...
        output_tokens_price = self.get_output_token_cost(model_name, output_tokens)
